...
public function herramientaDisponible(){
 ...
 if($this->con->error){
  ...
  $dataTable = null;
      ...
      $sql= "select herra.idHerramienta, herra.nombre, c.nombreCategoria, estado.estado,dp.idDetallePrestamo, dp.fechaInicioPrestamo, dp.fechaFinPrestamo, dp.fechaEntrega, dp.idPrestamo
...
$teracion=0;
      while($fila=$res->fetch_assoc())
      {
        ...
        $dataTable.= "<tr>
                         ...
                         <a class='btn btn-success btn-lg' href=\"javascript:llenarDetalle(
                          ".$fila['idDetallePrestamo'].",
                          '".$fila['fechaInicioPrestamo']."',
                          '".$fila['fechaFinPrestamo']."',
                          '".$fila['fechaEntrega']."',
                          ".$fila['idHerramienta'].",
                          ".$fila['idPrestamo'].");\"> Seleccionar</a>
                         <input type='submit' name='btnEliminarH' value='Eliminar' class='btn btn-danger btn-lg'>
                         <input type='submit' name='btnInsertarH' value='Insertar' class='btn btn-danger btn-lg'>
