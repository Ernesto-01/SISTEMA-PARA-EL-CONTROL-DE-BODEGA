...
public function cantidadHerramientasPrestadas($id){
  ...
  return $data;
    ...
    $dataTable = null;
    		...
    		$sql= "select p.idPrestamo, p.descripcion, p.idEmpleadoEntrega, p.idEmpleadoRecive from prestamo as p where  p.activo=1;";
    		$res= $this->con->query($sql);

    		$dataTable.= "<table border=1 >
                          <form method='post' action='frmDetallePrestamo.php'>
        		             <tr>
        		              <th>ID</th>
        		              <th>Descripcion</th>
    		...
    		while($fila=$res->fetch_assoc())
    		{
    			...
    			$dataTable.= "<tr>
                           ...
                           <input type='hidden' name='a' value='".$fila['idPrestamo']."'>
                           <input type='hidden' name='b' value='".$fila['descripcion']."'>
                           <input type='hidden' name='c' value='".$fila['idEmpleadoEntrega']."'>
                           <input type='hidden' name='d' value='".$fila['idEmpleadoRecive']."'>
      			               <td>
                               ...
                               <input type='submit' name='btnAgregarH' value='Agregar Herramientas'>
      			               </td>
    			             </tr>";
    		}
        } catch (\Exception $e) {
